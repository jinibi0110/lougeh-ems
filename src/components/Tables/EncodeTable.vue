...
<div class="encode">
  <div>
    <md-dialog :md-active.sync="showDialog">
      <md-dialog-title>Encode Grade</md-dialog-title>
      ...
      <md-dialog-actions>
        ...
        <md-button class="md-info" @click="addGrade()">Submit</md-button>
      </md-dialog-actions>
    </md-dialog>
  </div>
  <div ID="data">
    <br>
    <md-table v-model="grades" md-sort="sid" md-sort-order="asc" md-card md-fixed-header :table-header-color="tableHeaderColor">
      <md-table-toolbar>
      ...
      <md-table-empty-state
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
          <md-table-cell md-sort-by="id" md-label="Student ID">{{ item.sid }}</md-table-cell>
          <md-table-cell md-label="Grade">{{ item.grade }}</md-table-cell>
          <md-table-cell md-label="Mark">{{ item.mark }}</md-table-cell>
          <md-table-cell md-label="Action">{{ item.action }}
            <md-button class="md-success md-raised" @click="showDialog = true">Encode</md-button>
          </md-table-cell>
        </md-table-row>
    </md-table>
  </div>
  ...
  data() {
   ...
   methods:{
     newUser () {
      ...
      searchOnTable () {
        this.searched = searchByName(this.grades, this.search)
      }
    },
    created () {
      this.searched = this.grades
    },

    editGrade() {
          this.grades.map(student => {

            if(student.sid === this.grades.sid){

              student.grade = this.grades.grade,
              student.mark = this.grades.mark
            }
          })
